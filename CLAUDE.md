...
### One search path, with a scrape-based fallback
...
When the OpenRouter call hard-fails (exception after `call_with_backoff` retries exhaust, or a 200 with zero usable annotations), `web_search` falls back to scraping `https://html.duckduckgo.com/html/` — the no-JS DuckDuckGo endpoint — via regex (`_duckduckgo_fallback`), not an HTML parser, since this path only fires on primary-search failure and doesn't justify a new dependency. It decodes DuckDuckGo's `uddg` redirect param back to the real URL and emits the same `[SRC] url | title` / `EXCERPT:` shape, so downstream parsing in `app/nodes/parsing.py` doesn't know which engine answered. A successful fallback counts as one search against `UsageCollector`. Tests inject `fallback_transport=` (an `httpx.MockTransport`) the same way `transport=` covers the primary call — any test exercising a primary failure path must also pass a `fallback_transport` that fails, or it silently reaches the real network.

### Search/fetch cache

[app/tools/cache.py](app/tools/cache.py)'s `TTLCache` (30 min TTL, 500-entry LRU cap, in-memory) sits behind two process-lifetime singletons, `search_cache` and `fetch_cache`, wired into `web_search`/`web_fetch` only by `build_research_agent` — the factories themselves default `cache=None` (off), so nothing outside `app/agent.py` opts in implicitly. Because the singletons live at module scope rather than inside `build_graph`/`build_research_agent` (both rebuilt per turn), a hit survives across research iterations *and* across turns in the same process — this is what makes it useful for a long-running `uvicorn` process, not just a single CLI run. Only successful results are cached (`SEARCH_RESULTS`/extracted page text); `SEARCH_ERROR`/`FETCH_ERROR` responses are never cached, so a transient failure doesn't get stuck for the TTL window. A search cache hit does not call `usage.add` — no OpenRouter request happened, so it must not appear in the `searches`/cost METRICS.
...
- `make_web_fetch(cfg, transport=...)` and `make_web_search(settings, transport=..., fallback_transport=...)` take an `httpx.MockTransport` — `fallback_transport` covers `web_search`'s DuckDuckGo scrape path, exercised only when `transport`'s response is a hard failure.
...
- `make_web_search(..., cache=...)` and `make_web_fetch(..., cache=...)` take an `app.tools.cache.TTLCache` and default to `None` (caching off) — tests that need to assert on cache behavior pass a fresh `TTLCache()` instance; everything else is unaffected since the default keeps the old no-cache behavior.
